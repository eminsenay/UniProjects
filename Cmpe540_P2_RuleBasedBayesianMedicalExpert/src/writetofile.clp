...
=>
  ...
  (if (> ?prob 0.95) then
    (bind ?yuzde (* ?prob 100))
    (printout out crlf crlf "You are with " ?yuzde "% " ?illnessName crlf)
    (printout out "If you want to continue, continue aswering; o/w press a" crlf crlf)
...
=>
  (if (> ?prob 0.80) then
    ...
    (printout out crlf "You are with " ?yuzde "% " ?illnessName crlf)
    (bind ?*foundIllness* 1))
  )
  (retract ?factNo1)
  (assert (finished 1))
...
=>
  (if (eq ?*foundIllness* 0) then
    ...
    (printout out crlf "I am not very sure of your illness." crlf)
    (printout out "No illness has more probability than 0.8" crlf)
  )
)
...
=>
  ...
  (printout out ?symptomquestion)
  (printout out " (yES/nO/aBORT) ")
  (bind ?answer (read))
  (printout out ?answer crlf)              
  (if (eq ?answer y) then
