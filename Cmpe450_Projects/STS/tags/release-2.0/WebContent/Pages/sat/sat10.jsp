...
//	SECURITY CODE BEGIN
	...
	if (session.getAttribute("securityEnabled").equals("true") && result3.next()) {
		...
		String security_documentContent = request.getParameter("document_content");
		String security_date = request.getParameter("securityDate");
		Security.addSignature(userID, requestFormID,security_date,security_signature,security_documentContent, historyID,Security.SIGN_FORMAT_SAT10);
		//SECURITY CODE ENDS
	}
...
<SCRIPT language="JavaScript">
	...
	<!-- SECURITY ONAY ICIN BU JAVASCRIPT FONK. CAGIRACAK -->
		...
		function onayDugmesi2() {
			
			var result=true;
			var deneme = document.onayFormu["kazanan0"].value;
			if (deneme == "-")
			{
				result = false;
			}
		   ...
		   if (result) {
		       ...
		       onayDugmesi();
				//parent.leftFrame.securityOnayDugmesi();
		 		...
		 		alert("Lütfen Uygun Gorulen Teklifi seçiniz");
		 	}
		}
		
		//security kismi cagiriyor ayrica cagirmayiniz
		...
		function teklifekle(){
			 ...
			 if(document.onayFormu.f1.value.length > 0) {
				 if(document.onayFormu.f2.value.length > 0) {         
					 if (validateElement(document.onayFormu.f3,true)){
						document.onayFormu.onay.value = "ekle";
						<!-- formun submiti cagrilarak sayfa kendisini cagiriyor -->
						document.onayFormu.submit();
					}
				}
				else{
					alert("Lutfen firma adresi giriniz");
				}
			}
			else{
				alert("Lutfen firma adi giriniz");
			}
		}
...
<form name="onayFormu" action="sat10.jsp" method="post">
  <div align="center">
...
//		mevcut tutar
        ...
        <td width="54%"><table width="100%"  border="1" bordercolor="#CCCCFF" class="tablefont">
          ...
          <tr>
            ...
            <td width="67%"><label><span id='security<%=securityIndex++ %>'><%=mevcutTutar%></span></label></td>
          </tr>
        </table></td>
      </tr>
    </table>
    ...
    <table width="750"  border="1" bordercolor="#CCCCFF" class="tablefont">
      <tr>
        <td width="19%">&nbsp;</td>
        <td width="16%">&nbsp;</td>
        <td width="15%">&nbsp;</td>
      ...
      <tr>
        ...
        <td><div align="center"><b>Uygun Teklif</b></div></td>
        </tr>
      <%
      _request.previous();
      ...
      for(j=0;_request.next();j++) {
	      ...
	      <tr>
	        ...
	        <td><div align="center"><select name="kazanan<%=j%>" id='security<%=securityIndex++%>'>
	        <%
	        ResultSet rs2= db.executeQuery("select * from ISTEK_TEKLIF where IstekNo='"+istekno+"'");
	      	while(rs2.next()){
	      	
	      	%>
	      	<option value="<%=rs2.getString("IstekTeklifNo")%>" ><%=rs2.getString("IstekTeklifNo")%></option>
	      	<%} %>
   ...
   <!-- kullanıcı tahmini bir fiyat girecek -->
   <br><div align="center">

	<table width="750"  border="0" bordercolor="#CCCCFF" class="tablefont">
	<tr>
        <td width="19%">&nbsp;</td>
        <td width="16%">&nbsp;</td>
        <td width="15%">&nbsp;</td>
      ...
      <tr>
        ...
        <tr>
      	<td><div align="center"><select name="istekid">
      	
      	<%System.out.println("Size= "+ names.size()); 
      	for(int i=0; i< names.size();i++){
      	System.out.println(i+": "+names.get(i));
      	%>
      	<option value="<%out.print(names.get(i));%>" selected><%out.print(names.get(i)); %></option>
      	<%} %>
      	</div></td> 
         <td><div align="center"><input type="text" name="f1"></div></td>
       
        <td><div align="center"><input type="text" name="f2"></div></td>
        <td><div align="center"><input type="text" name="f3" style="text-align: right;" validator="YTL">YTL</div></td>
        <td><div align="center"><input type="button" value="Teklif Ekle" onclick="teklifekle()" name="ek1" ></div></td>
  ...
  <br><div align="center">
  	<input type="button" value="Onayla" onClick="onayDugmesi2()" language="JavaScript">
