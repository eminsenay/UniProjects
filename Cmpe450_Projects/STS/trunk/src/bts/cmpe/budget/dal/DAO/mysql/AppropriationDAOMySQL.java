...
public class AppropriationDAOMySQL extends DAObase implements AppropriationDAO {
	 ...
	 * @throws ApprRedefined Duplicate record with same index number.
	 */
	public final void insertMainAppropriation(final BudgetTO budget, 
		...
		try {
			...
			if (numTotalAppr >= bt.getNumMainAppr()) {
				...
				if (appr.getDate() == null) {
					prep.setNull(FOUR, java.sql.Types.DATE);
				} else {
			...
			if (e.getErrorCode() == ApprRedefined.ERR_CODE) {
				throw new ApprRedefined(bt);
			}
		} finally {
			try {
				prep.close();
	 ...
	 */
	public final void insertAddAppropriation(final BudgetTO budget, 
			final AppropriationTO appr) throws NoSuchBudgetException, 
			ApprRedefined {
		PreparedStatement prep = null;
		BudgetTO bt = null;
		ResultSet rs = null;
