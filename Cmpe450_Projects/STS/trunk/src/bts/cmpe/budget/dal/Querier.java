...
public final class Querier {
	 ...
	 *             In case that no matching budget exists in database.
	 */
	...
	/**
	 * @param budgetId
	 *            Primary key of a row in core_budget table.
	 * @param conn
	 *            Connection obj
	 * @return Vector of appropriations given to a budget.
	 * @throws SQLException
	 *             DB related exceptions
	 * @throws NoApprDefined
	 *             In case that no appropriation exists for the budget
	 */
	...
	/**
	 * @param budgetId
	 *            Primary key of a row being a std expense budget in core_budget
	 *            table.
	 * @param conn
	 *            Connection obj
	 ...
	 * @return NextInventory Sequence
	 * @throws SQLException DB Exception
	 */
	public static int getInvetorySequence(final int id, final Connection conn)
			throws SQLException {
		final String query = "SELECT inventory_indx "
				+ "FROM AUX_INVENTORY_INDX "
				+ "WHERE budget_fk = ? FOR UPDATE";
		final String updateSeq = "UPDATE AUX_INVENTORY_INDX "
				+ "SET inventory_indx = inventory_indx + 1 where budget_fk = ?";
		...
		prep.setInt(PARAM_ONE, id);

		rs = prep.executeQuery();

		if (rs.next()) {
			inventoryNo = rs.getInt(PARAM_ONE);
			prep = conn.prepareStatement(updateSeq);
