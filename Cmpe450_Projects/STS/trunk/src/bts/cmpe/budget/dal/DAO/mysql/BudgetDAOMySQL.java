...
public class BudgetDAOMySQL extends DAObase implements BudgetDAO {
	...
	public final BudgetTO getBudgetDetail(final BudgetTO bgdt)
		...
		BudgetTO bt = null;
		try {
			bt = Querier.selectBudgetForUpdate(bgdt.getInstCode(), bgdt
					.getFuncCode(), bgdt.getEcoCode(), super.getConn());
	...
	public final Vector<BudgetTO> getEconomicCodes(final BudgetTO budget)
		...
		try {
			...
			while (rs.next()) {
				budgetVect.addElement(new BudgetTO(rs.getInt(ONE), rs
						.getInt(TWO), rs.getDouble(THREE), rs.getInt(FOUR), rs
						.getString(FIVE), rs.getString(SIX), rs
						.getString(SEVEN)));
			}
	...
	public final void insertBudget(final BudgetTO budget)
		...
		final String createInvIndx = "insert into AUX_INVENTORY_INDX "
				+ "values(1, ?)";
		...
		try {
			...
			if (budget.getFuncCode() == null) {
				prep.setNull(FIVE, java.sql.Types.VARCHAR);
			} else {
				prep.setString(FIVE, budget.getFuncCode());
			}

			if (budget.getEcoCode() == null) {
				prep.setNull(SIX, java.sql.Types.VARCHAR);
			} else {
				prep.setString(SIX, budget.getEcoCode());
			}

			if (budget.getInstCode() == null) {
				prep.setNull(SEVEN, java.sql.Types.VARCHAR);
			} else {
			...
			if (rs.next()) {
				prep = super.getConn().prepareStatement(createInvIndx);
				prep.setInt(ONE, rs.getInt(ONE));
				prep.execute();
			}
		} catch (SQLException ex) {
	...
	public final void updateBudget(final BudgetTO old, final BudgetTO newB)
		...
		final String dml = "UPDATE core_budget SET budget_type_i = ?, "
				+ "num_main_appr_i = ? " + "WHERE budget_id_i = ?";
