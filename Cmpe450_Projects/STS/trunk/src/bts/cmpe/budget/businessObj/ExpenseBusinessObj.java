...
public class ExpenseBusinessObj {
	 ...
	 * @param inventoryNo Inventory ID.
	 * @throws Exception Exception to JSP.
	 */
	public final void approveExpenditure(final BudgetTO budget,
			final int inventoryNo) throws Exception {
		Transaction tr = null;
		
		try {
			tr = Transaction.create();
			ExpenseDAOMySQL dao = new ExpenseDAOMySQL(tr);
			
			dao.approveExpenditure(budget, inventoryNo);
			tr.commit();
	 ...
	 * @throws Exception for all kind of exception
	 */
	...
	throws Exception {
		...
		try {
			...
			trans.updateStdExpenditure(budget, expense);
			tr.commit();
		} catch (Exception e) {
			throw e;
		} finally {
