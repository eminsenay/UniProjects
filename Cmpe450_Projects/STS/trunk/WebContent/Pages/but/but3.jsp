...
<%@page import="bts.cmpe.budget.transfer.BudgetTO"%>
<%@page import="bts.cmpe.budget.businessObj.ExpenseBusinessObj"%>
<%@page import="bts.cmpe.budget.transfer.ExpenseStdTO"%>
<%@page import="java.text.DateFormat"%>
<%@page import="bts.cmpe.budget.transfer.ExpenseBaseTO"%>
...
ResultSet rsBts = db.executeSP("IstekFisiGoster_sp",new Object[]{requestFormID}).getResult();
rsBts.next();

String inst = rsBts.getString("KurumsalKod");;
String func = rsBts.getString("FonksiyonelKodu");
String eco  = rsBts.getString("AnaKalemKodu");

String dateStr = rsBts.getString("OlusturmaTarihi");		
Date requestDate = Date.valueOf(dateStr.split(" ")[0]);
double reqAmount = (toplam==null)?0:Double.parseDouble(toplam.replace(',','.'));
String materialType = rsBts.getString("UrunAdi");
String reqEcoCode = rsBts.getString("KalemKodu");
String unit = "Bilgisayar Muh.";
BudgetTO cBudget = new BudgetTO(inst,func,eco);

ExpenseStdTO stdExpense = new ExpenseStdTO();
stdExpense.setRequestDate(requestDate);
stdExpense.setRequestAmount(reqAmount);
stdExpense.setEcoCode(reqEcoCode);
stdExpense.setMaterialType(materialType);
stdExpense.setUnit(unit);
stdExpense.setRealizationAmount(reqAmount);
stdExpense.setStatus(ExpenseBaseTO.WAITING);
stdExpense.setToWhom("");
stdExpense.setRealizationDate(requestDate);
stdExpense.setRealizationId(0);
...
if(status!=null){
	if(status.equals("onay")) {
		// entegre bts
		expenseBO.addStdExpense(cBudget,stdExpense);
...
<div align="center">
  <table width="79%"  border="0" bordercolor="#CCCCFF" class="tablefont">
    <tr>
      ...
      <%
		...
		// ana kalem no
		String anaKalemKodu = rs.getString("AnaKalemKodu");
		//proje no
      ...
      <td width="60%"><table width="100%"  border="1" bordercolor="#CCCCFF" class="tablefont" style="font-weight: normal;">
        ...
        <input id='security<%=securityIndex++ %>' type="hidden" name="bsirano" value="<%=BSiraNo %>">
      </table></td>
    </tr>
  </table>
	<br>
  ...
  <table width="79%"  border=" bordercolor="#CCCCFF" class="tablefont2">
    ...
    // bts entegroterleri konusuyor:
		...
		BudgetTO currentBudget = new BudgetTO(kurumsalKod,fonksiyonelKod,anaKalemKodu);
		...
		MevcutTutar = "" + (periodicVal + additionalVal + transferVal - expenseVal);
		MevcutTutar = MevcutTutar.replace('.',',');
		//double kalanTutar = 0;
		//double kalanTutar = currentBudget.getNetBudget() - Double.parseDouble(toplam);
    ...
    <tr>
    	...
    	<td colspan=2 align="left"><input id="kalan" type="text" name="kalan" value="" style="width: 100%; text-align: right;" disabled=""></td>
    </tr>
  </table>
 <table width="79%">
 <tr>
