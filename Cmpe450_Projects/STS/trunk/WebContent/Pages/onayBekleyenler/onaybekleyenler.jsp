...
<title>
Boğazi&ccedil;i &Uuml;niversitesi Sat&#305;n Alma Sistemi
</title>
...
<table border="0" cellspacing="0" cellpadding="0" width="600">
 ...
 %>
		...
		<%
			...
			if (userInfo == null)
			{
				out.println("Ana menüye gitmek için lütfen önce giriş yapınız.");
			}
			else
			{
			...
			if (GDC || DDC) {
				...
				if (GDC) {
					...
					if (counter > 0) {
					%>
					<p>Limit altına düşen <%=counter%> ürün bulunmaktadır.</p>
					<% 
				...
				if (DDC) {
					...
					//String birimAdi = Stok.getBirimIsmi(userInfo.getgroupID());
					//ArrayList<String> urun_names = new ArrayList<String>();
					//urun_names = Stok.getUrunNames();
					while(limitAlti.next())
					{
			...
			ResultSet rs;
			String groupId = userInfo.getgroupID();
			if(groupId.equals("000002-GRP")  || groupId.equals("000005-GRP") || groupId.equals("000007-GRP")){
				rs = db.executeQuery("call TumIstekFisleri_sp('"
						+ userInfo.getUserid() + "')");
			}
			else{
				rs = db.executeQuery("call KullaniciIstekFisi_sp('"
						+ userInfo.getUserid() + "')");
			}
...
%>
					<p>Sistemde bekleyen istek fişi bulunmamaktadır.</p>
					<%
		...
		<table bgcolor="#FFFFFF" width="500" border="2" bordercolor="#CCCCFF"
			cellspacing="0" cellpadding="0" align=left class="tablefont3">

			<tr>
				<td colspan="6" align="center"><font size="2"><label>Onay
				Bekleyen &#304;stekler</label><br>
				<label>(Detay görmek i&ccedil;in t&#305;klay&#305;n&#305;z)</label></font></td>
			</tr>
			<tr>
				<td></td>
				<td><font size="2">Fi&#351; Kodu</font></td>
				<td><font size="2">İsteği Yapan</font></td>
				<td><font size="2">İstek Fi&#351;i No</font></td>
				<td><font size="2">Tarih</font></td>
				<td><font size="2">Hangi Sayfada</font></td>
			</tr>
			<%
					//		        User userInfo = (User)(session.getAttribute("userClass"));
					//eski
			...
			<tr>
				...
				<%
				if (!rs.isFirst()) {
				%> <input type="radio" name="requestID"
					value="<%=rs.getString("IstekFisiNo")%>"> <%
 		} else {
 		resultSetisEmpty = 1; //buraya aldım çünkü tekrar tekrar gereksiz yere yapmasını istemiyorum
 %> <input type="radio" name="requestID"
					value="<%=rs.getString("IstekFisiNo")%>" checked> <%
 }
 %>
				</td>
				<td style="FONT-WEIGHT : normal"><%=rs.getString("IstekFisiKodu")%>
				</td>
				<td style="FONT-WEIGHT : normal"><%=rs.getString("Isteyen")%>
				</td>
				<td style="FONT-WEIGHT : normal"><%=rs.getString("IstekFisiNo")%>
				</td>
				<td style="FONT-WEIGHT : normal"><%=rs.getString("OlusturmaTarihi")%>
				</td>
				<td style="FONT-WEIGHT : normal"><%=rs.getString("SayfaAdi")%>
				</td>
			</tr>
			<%
			}
			...
			<tr>
				...
				%> <input type="submit" class="tablefont3"
					value="Detay G&#246;ster"> <%
 }
 %>
				</td>
