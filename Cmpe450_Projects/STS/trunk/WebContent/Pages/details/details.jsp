...
&& request.getParameter("onay").equals("onaylandi")) {
	...
	ResultSet result3 = DatabaseConnection.executeQuery("call IstekAktar_sp('"+requestFormID+"','"+nextGroupID+"','"+nextPageCode+"','"+userID+"','"+yorum+"','1')");
	...
	String nextPageCode = "";
	String userID = ((User)session.getAttribute("userClass")).getUserid();
	ResultSet result3 = DatabaseConnection.executeQuery("call IstekAktar_sp('"+requestFormID+"','"+nextGroupID+"','"+nextPageCode+"','"+userID+"','"+yorum+"','0')");
	//bu sp'nin hatal? dönmesi olas?l?g?n? kontrol et
	//basar?l? oldugunu varsayarsak
	//sessiondan aman requestFromID'yi remove et
	session.removeAttribute("requestFormID");
